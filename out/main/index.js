...
function readWavInfoNode(buf) {
  ...
  if (buf.toString("ascii", 0, 4) !== "RIFF" && buf.toString("ascii", 0, 4) !== "RF64") return null;
...
async function doTransfer(project, mixerStates, filePaths, sender) {
  ...
  for (const pl of project.playlists) {
    ...
    const expectedSongFolders = /* @__PURE__ */ new Set();
    for (const song of songs) {
      const sName = sanitizeName(song.name);
...
ipcMain.handle("filecache:save", async (_, cacheObj) => {
  try {
    writeFileSync(fileCachePath(), JSON.stringify(cacheObj), "utf-8");
    return true;
  } catch {
    return false;
  }
...
ipcMain.handle("filecache:load", async () => {
  try {
    return JSON.parse(readFileSync(fileCachePath(), "utf-8"));
  } catch {
    return null;
  }
});
...
ipcMain.handle("pick:relink", async (_, isMidi) => {
  const filters = isMidi ? [{ name: "MIDI", extensions: ["mid", "midi"] }] : [{ name: "WAV Audio", extensions: ["wav"] }];
  const { canceled, filePaths } = await dialog.showOpenDialog(mainWindow, {
  ...
  if (!isMidi) {
    try {
      ...
      return { filePath, nameNoExt, stereo: false, warnings: [] };
    }
  }
  return { filePath, nameNoExt };
});
...
function readFirmwareCache() {
  try {
    return JSON.parse(readFileSync(firmwareCachePath(), "utf-8"));
  } catch {
    return null;
  }
}
function writeFirmwareCache(data) {
  try {
    writeFileSync(firmwareCachePath(), JSON.stringify(data, null, 2), "utf-8");
  } catch {
  }
}
function parseFirmwareVersion(filename) {
  ...
  if (!m) return null;
  return m[3] ? `${parseInt(m[1])}.${parseInt(m[2])}.${parseInt(m[3])}` : `${parseInt(m[1])}.${parseInt(m[2])}`;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = mod.request(url, { method, headers: { "User-Agent": "CIdoru/1.4" } }, (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        const loc = res.headers.location.startsWith("http") ? res.headers.location : `https://idoru.live${res.headers.location}`;
...
ipcMain.handle("firmware:check", async () => {
  ...
  const knownHigh = firmwares.length > 0 ? firmwares.reduce((best, f) => versionGt(f.version, best) ? f.version : best, firmwares[0].version) : null;
  ...
  for (let offset = 0; offset <= 15; offset++) {
    ...
    if (version && !probed.has(version)) {
      probed.add(version);
      probePromises.push(
        httpFetch(url, "HEAD").then((res) => res.status === 200 ? { url, filename, version } : null).catch(() => null)
      );
    }
  }
...
ipcMain.handle("firmware:download", async (event, url, version) => {
  try {
    mkdirSync(firmwareBinDir(), { recursive: true });
  } catch {
  }
  const filename = url.split("/").pop();
  ...
  const https = await import("https");
  const http = await import("http");
  const downloadStream = (url2, dest) => new Promise((resolve, reject) => {
    const mod = url2.startsWith("https") ? https.default ?? https : http.default ?? http;
    const req = mod.request(url2, { headers: { "User-Agent": "CIdoru/1.4" } }, (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        const loc = res.headers.location.startsWith("http") ? res.headers.location : `https://idoru.live${res.headers.location}`;
  ...
  try {
    ...
    const cache = { version, filename, path: destPath, downloadedAt: (/* @__PURE__ */ new Date()).toISOString(), url };
...
ipcMain.handle("firmware:pick", async () => {
  ...
  const cache = { version, path: filePaths[0], downloadedAt: (/* @__PURE__ */ new Date()).toISOString(), manual: true };
  writeFirmwareCache(cache);
  return cache;
});
...
ipcMain.handle("firmware:clearCache", async () => {
  try {
    writeFileSync(firmwareCachePath(), JSON.stringify(null), "utf-8");
  } catch {
  }
  return true;
});
...
ipcMain.handle("firmware:writeToSd", async (_, sdRootPath, firmwarePath) => {
  try {
    ...
    const { readdirSync: readdirSync2, unlinkSync } = await import("fs");
    const existing = readdirSync2(updateDir);
    for (const f of existing) {
      try {
        unlinkSync(join(updateDir, f));
      } catch {
      }
    }
