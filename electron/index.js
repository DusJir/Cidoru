...
async function doTransfer (project, mixerStates, filePaths, sender) {
    ...
    for (const pl of project.playlists) {
        ...
        await writeFile(join(plDir, `${plName}.txt`), slTxt, 'utf-8')
        sender.send('transfer:progress', `✓ ${plName}/${plName}.txt`)
        ...
        for (const song of songs) {
            ...
            await writeFile(join(songDir, `${sName}.txt`), sTxt, 'utf-8')
            sender.send('transfer:progress', `  ✓ ${plName}/${sName}/${sName}.txt`)
            ...
            for (let i = 0; i < slots.length; i++) {
                ...
                if (srcPath && existsSync(srcPath)) {
                    ...
                    await writeFile(join(songDir, `${sl.fileName}.wav`), bytes)
                    sender.send('transfer:progress', `  ✓ ${sl.fileName}.wav`)
                } else {
                    missing.push(`${song.name} — F${i + 1}: ${sl.fileName}.wav`)
                    sender.send('transfer:progress', `  ⚠ ${sl.fileName}.wav — file not found`)
                }
            }
            ...
            if (song.midiFile) {
                ...
                if (srcPath && existsSync(srcPath)) {
                    const bytes = await readFile(srcPath)
                    await writeFile(join(songDir, `${song.midiFile}.mid`), bytes)
                    sender.send('transfer:progress', `  ✓ ${song.midiFile}.mid`)
                } else {
                    missing.push(`${song.name} — MIDI: ${song.midiFile}.mid`)
                    sender.send('transfer:progress', `  ⚠ ${song.midiFile}.mid — file not found`)
                }
            }
        }
    }
...
// ── P-1 file generation (duplicated from ChannelStrip for main process) ──
...
function generateSongFile (song, state) {
    ...
    const R2C      = [1, 2, 3, 4, 5, 6, 0]

    let txt = `"${song.name}"\n`
    txt += `Level- ${Math.round(dbToNorm(mod, -20, 20) * 100)}\n`
    ...
    for (let i = 0; i < 6; i++) {
        ...
        if (sl?.fileName) {
            ...
            const ch = sl.stereo ? 'Stereo' : 'Mono'
            txt += `"${sl.fileName}" ${label} ${ch}\n`
        }
    }
